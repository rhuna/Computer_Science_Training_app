...
MainWindow::MainWindow(QWidget* parent)
    ...
    , m_progress(QDir(QStandardPaths::writableLocation(QStandardPaths::AppDataLocation)).filePath("progress_v27.sqlite"))
{
    ...
    applyAppStyle();
    refreshAllTabs();
    setWindowTitle("CS Bootcamp Desktop v27 - Learning / Testing Course Mode");
...
{
    // v27 keeps the sidebar object for compatibility with older helper methods,
...
QWidget* MainWindow::createGuidedBootcampPage()
{
    ...
    outerLayout->addWidget(header("CS Bootcamp Desktop v27 - Learning / Testing Course Mode"));

    QLabel* intro = new QLabel(
        "v27 keeps the one-course experience, but divides the workflow into two clean tabs: "
        "Learning is where you understand the concept, get coached, and plan. Testing is where you code, run, verify, quiz, and prove mastery.");
    ...
    chooserLayout->addWidget(smallTitle("Course module"), 0, 0);
    chooserLayout->addWidget(m_moduleSelector, 1, 0);
    chooserLayout->addWidget(smallTitle("Coding language"), 0, 1);
    chooserLayout->addWidget(m_runnerLanguage, 1, 1);
    outerLayout->addWidget(chooser);

    QTabWidget* courseTabs = new QTabWidget;
    courseTabs->setObjectName("courseTabs");

    // ---------------------------------------------------------------------
    // LEARNING TAB
    // ---------------------------------------------------------------------
    QWidget* learningTab = new QWidget;
    QVBoxLayout* learningOuter = new QVBoxLayout(learningTab);
    learningOuter->setContentsMargins(10, 10, 10, 10);
    learningOuter->setSpacing(12);

    QScrollArea* learningScroll = new QScrollArea;
    learningScroll->setWidgetResizable(true);
    learningScroll->setFrameShape(QFrame::NoFrame);
    QWidget* learningPage = new QWidget;
    QVBoxLayout* learningLayout = new QVBoxLayout(learningPage);
    learningLayout->setSpacing(14);
    learningLayout->setContentsMargins(8, 8, 8, 8);

    QFrame* lessonCard = card();
    QHBoxLayout* lessonLayout = new QHBoxLayout(lessonCard);
    QVBoxLayout* lessonLeft = new QVBoxLayout;
    QVBoxLayout* lessonRight = new QVBoxLayout;
    m_lessonList = new QListWidget;
    m_lessonList->setMinimumWidth(340);
    lessonLeft->addWidget(smallTitle("1. Learn one lesson at a time"));
    lessonLeft->addWidget(m_lessonList, 1);
    m_lessonTitle = smallTitle("Lesson");
    m_lessonBody = readOnlyText(360);
    QPushButton* completeLesson = new QPushButton("Mark lesson understood");
    lessonRight->addWidget(m_lessonTitle);
    lessonRight->addWidget(m_lessonBody, 1);
    lessonRight->addWidget(completeLesson);
    lessonLayout->addLayout(lessonLeft, 0);
    lessonLayout->addLayout(lessonRight, 1);
    learningLayout->addWidget(lessonCard);

    QFrame* helperCard = card();
    QVBoxLayout* helperLayout = new QVBoxLayout(helperCard);
    helperLayout->addWidget(smallTitle("2. Advanced helper - Google-interviewer style coaching"));
    m_helperContextLabel = new QLabel;
    m_helperContextLabel->setWordWrap(true);
    helperLayout->addWidget(m_helperContextLabel);
    ...
    helperLayout->addLayout(timerRow);
    ...
    helperLayout->addWidget(m_helperPhaseSelector);
    m_helperBody = readOnlyText(260);
    helperLayout->addWidget(m_helperBody);

    QHBoxLayout* helperButtons = new QHBoxLayout;
    QPushButton* startTimer = new QPushButton("Start real-time timer");
    QPushButton* pauseTimer = new QPushButton("Pause");
    QPushButton* resetTimer = new QPushButton("Reset");
    QPushButton* popup = new QPushButton("Show coaching pop-up");
    QPushButton* nextPhase = new QPushButton("Next phase");
    QPushButton* jumpToTestingFromHelper = new QPushButton("Go to Testing");
    ...
    helperButtons->addWidget(jumpToTestingFromHelper);
    helperLayout->addLayout(helperButtons);
    learningLayout->addWidget(helperCard);

    QFrame* mentalModelCard = card();
    QHBoxLayout* mentalLayout = new QHBoxLayout(mentalModelCard);
    QVBoxLayout* patternLayout = new QVBoxLayout;
    patternLayout->addWidget(smallTitle("3. Problem-solving pattern"));
    ...
    m_problemCoachBody = readOnlyText(320);
    patternLayout->addWidget(m_problemPatternSelector);
    patternLayout->addWidget(m_problemCoachBody, 1);

    QVBoxLayout* algorithmLayout = new QVBoxLayout;
    algorithmLayout->addWidget(smallTitle("4. Visual mental model"));
    ...
    m_algorithmComplexity = smallTitle("Algorithm model");
    m_algorithmOutput = readOnlyText(320);
    QPushButton* nextAlgo = new QPushButton("Next algorithm step");
    algorithmLayout->addWidget(m_algorithmSelector);
    algorithmLayout->addWidget(m_algorithmComplexity);
    algorithmLayout->addWidget(m_algorithmOutput, 1);
    algorithmLayout->addWidget(nextAlgo);

    mentalLayout->addLayout(patternLayout, 1);
    mentalLayout->addLayout(algorithmLayout, 1);
    learningLayout->addWidget(mentalModelCard);

    learningScroll->setWidget(learningPage);
    learningOuter->addWidget(learningScroll);

    // ---------------------------------------------------------------------
    // TESTING TAB
    // ---------------------------------------------------------------------
    QWidget* testingTab = new QWidget;
    QVBoxLayout* testingOuter = new QVBoxLayout(testingTab);
    testingOuter->setContentsMargins(10, 10, 10, 10);
    testingOuter->setSpacing(12);

    QScrollArea* testingScroll = new QScrollArea;
    testingScroll->setWidgetResizable(true);
    testingScroll->setFrameShape(QFrame::NoFrame);
    QWidget* testingPage = new QWidget;
    QVBoxLayout* testingLayout = new QVBoxLayout(testingPage);
    testingLayout->setSpacing(14);
    testingLayout->setContentsMargins(8, 8, 8, 8);

    QFrame* exerciseCard = card();
    QHBoxLayout* exerciseLayout = new QHBoxLayout(exerciseCard);
    QVBoxLayout* exerciseLeft = new QVBoxLayout;
    QVBoxLayout* exerciseRight = new QVBoxLayout;
    m_exerciseList = new QListWidget;
    m_exerciseList->setMinimumWidth(340);
    exerciseLeft->addWidget(smallTitle("1. Choose the connected exercise"));
    exerciseLeft->addWidget(m_exerciseList, 1);
    m_exerciseTitle = smallTitle("Exercise");
    m_exerciseBody = readOnlyText(210);
    m_exerciseCode = new QTextEdit;
    m_exerciseCode->setMinimumHeight(300);
    exerciseRight->addWidget(m_exerciseTitle);
    exerciseRight->addWidget(m_exerciseBody);
    exerciseRight->addWidget(smallTitle("2. Code editor - replace starter code with your own solution"));
    exerciseRight->addWidget(m_exerciseCode, 1);
    exerciseLayout->addLayout(exerciseLeft, 0);
    exerciseLayout->addLayout(exerciseRight, 1);
    testingLayout->addWidget(exerciseCard);

    QFrame* verifyCard = card();
    QVBoxLayout* verifyLayout = new QVBoxLayout(verifyCard);
    verifyLayout->addWidget(smallTitle("3. Build, run, test, and verify correctness"));
    m_runnerContext = new QLabel;
    m_runnerContext->setWordWrap(true);
    verifyLayout->addWidget(m_runnerContext);
    m_runnerCommandPreview = readOnlyText(80);
    verifyLayout->addWidget(m_runnerCommandPreview);
    ...
    QPushButton* verifyCorrect = new QPushButton("Verify Correct with real tests");
    QPushButton* stop = new QPushButton("Stop");
    QPushButton* jumpToLearningFromRunner = new QPushButton("Back to Learning");
    ...
    runButtons->addWidget(jumpToLearningFromRunner);
    verifyLayout->addLayout(runButtons);
    m_runnerOutput = new QTextEdit;
    m_runnerOutput->setMinimumHeight(260);
    verifyLayout->addWidget(m_runnerOutput);
    testingLayout->addWidget(verifyCard);

    QFrame* quizProgressCard = card();
    QHBoxLayout* quizProgressLayout = new QHBoxLayout(quizProgressCard);
    QVBoxLayout* quizLayout = new QVBoxLayout;
    m_quizList = new QListWidget;
    m_quizList->setMaximumHeight(130);
    m_quizTitle = smallTitle("4. Quiz checkpoint");
    m_quizBody = readOnlyText(150);
    m_quizChoices = new QListWidget;
    m_quizFeedback = new QLabel("Choose an answer, then check it.");
    m_quizFeedback->setWordWrap(true);
    QPushButton* checkQuiz = new QPushButton("Check quiz answer");
    quizLayout->addWidget(m_quizTitle);
    quizLayout->addWidget(m_quizList);
    quizLayout->addWidget(m_quizBody);
    quizLayout->addWidget(m_quizChoices);
    quizLayout->addWidget(checkQuiz);
    quizLayout->addWidget(m_quizFeedback);

    QVBoxLayout* progressLayout = new QVBoxLayout;
    progressLayout->addWidget(smallTitle("5. Progress evidence"));
    m_databaseBody = readOnlyText(330);
    QPushButton* recordDrill = new QPushButton("Record problem-solving drill");
    QPushButton* exportProgress = new QPushButton("Export progress summary");
    progressLayout->addWidget(m_databaseBody, 1);
    progressLayout->addWidget(recordDrill);
    progressLayout->addWidget(exportProgress);

    quizProgressLayout->addLayout(quizLayout, 1);
    quizProgressLayout->addLayout(progressLayout, 1);
    testingLayout->addWidget(quizProgressCard);

    testingScroll->setWidget(testingPage);
    testingOuter->addWidget(testingScroll);

    courseTabs->addTab(learningTab, "Learning + Coaching");
    courseTabs->addTab(testingTab, "Testing + Verification");
    outerLayout->addWidget(courseTabs, 1);
    ...
    connect(jumpToTestingFromHelper, &QPushButton::clicked, this, [courseTabs] { courseTabs->setCurrentIndex(1); });
    connect(jumpToLearningFromRunner, &QPushButton::clicked, this, [courseTabs] { courseTabs->setCurrentIndex(0); });
    ...
    connect(completeLesson, &QPushButton::clicked, this, [this]
    {
        if (Lesson* lesson = currentLesson())
        {
            m_progress.setCompleted(lesson->id, true);
            m_progress.recordStudyEvent(lesson->id, "lesson_completed", 25, "Completed lesson from v27 Learning tab.");
            refreshAllTabs();
        }
    });
    connect(checkQuiz, &QPushButton::clicked, this, [this]
    {
        ...
        if (correct)
        {
            ...
            m_progress.updateSkillMastery(currentModule() ? currentModule()->title : "Quiz", 70, "Quiz mastered inside v27 Testing tab.");
        }
        refreshAllTabs();
    });
    connect(nextAlgo, &QPushButton::clicked, this, [this] { nextAlgorithmStep(); });
    connect(recordDrill, &QPushButton::clicked, this, [this] { recordProblemSolvingDrill(); });
    connect(exportProgress, &QPushButton::clicked, this, [this] { exportProgressSummary(); });
...
QWidget* MainWindow::createDashboardPage()
{
    ...
    layout->addWidget(header("CS Bootcamp Desktop v27 - Learning / Testing Course Mode"));
...
QString MainWindow::lessonDetailText(const Lesson& lesson) const
{
    ...
    text += "HOW TO COMPLETE THIS OBJECTIVE USING THE v27 GUIDED PROBLEM-SOLVING LOOP\n";
...
QString MainWindow::quizSummaryText() const
{
    const Module* module = (m_currentModuleIndex >= 0 && m_currentModuleIndex < m_modules.size()) ? &m_modules[m_currentModuleIndex] : nullptr;
    if (!module) return {};
    return QString("Quiz bank for %1 contains %2 questions in v27. Questions now emphasize problem classification, pattern choice, debugging failed tests, complexity reasoning, and explaining correctness.").arg(module->title).arg(module->quiz.size());
}
...
{
    const QString base = QCoreApplication::applicationDirPath() + "/examples/v27_learning_testing_course_exercises";
...
void MainWindow::verifyRunnerOutput()
{
    ...
    m_lastRunnerOutput.clear();
    appendRunnerOutput("\n[VERIFY] v27 real verifier started. It checks behavior, then asks you to review the problem-solving process.\n");
...
void MainWindow::recordRunnerAttempt(const QString& status)
{
    ...
    m_progress.recordCodeAttempt(track->name, module->title, exercise->id, exercise->difficulty, commandForRunner("run"), m_lastRunnerOutput.left(500), status, "Recorded from v27 problem-solving course runner.");
}
...
void MainWindow::exportProgressSummary()
{
    ...
    QDir().mkpath(folder);
    QFile file(folder + "/v27_guided_problem_solving_progress_summary.md");
    if (file.open(QIODevice::WriteOnly | QIODevice::Text))
    {
        QTextStream out(&file);
        out << "# CS Bootcamp Desktop v27 Progress Summary\n\n";
        out << m_progress.exportMarkdownSummary();
        out << "\n\n## Code Attempts\n\n" << m_progress.codeAttemptMarkdown();
    }
    QMessageBox::information(this, "Exported", "Progress summary exported to progress_exports/v27_guided_problem_solving_progress_summary.md");
}
...
void MainWindow::recordProblemSolvingDrill()
{
    ...
    m_progress.recordStudyEvent(itemId, "problem_solving_drill", 20, "Completed v27 drill using pattern: " + pattern);
...
void MainWindow::refreshAllTabs()
{
    ...
    if (m_contentBody && module) m_contentBody->setPlainText("Current module content pack\n\n" + moduleSummaryText(*module) + "\n\nEvery lesson now includes a problem-solving loop, checklist how-to steps, practice instructions, and runner verification guidance. v27 adds deliberate practice patterns so learners build speed through repeatable thinking, not random guessing.");
