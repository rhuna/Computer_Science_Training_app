...
QVector<Module> BootcampContent::createDefaultModules()
{
    ...
    modules.push_back(Module{
        "m14_advanced_computer_science_course",
        "14 - Advanced Computer Science Course",
        "Advanced / Senior Foundations",
        "Integrated advanced lessons for learners who have completed the core bootcamp and need deeper professional CS: distributed systems, compilers, database internals, performance, advanced security, AI/RAG systems, and architecture tradeoffs.",
        {
            lesson("l_adv_distributed_systems", "Distributed Systems and Reliability", "Understand how multiple services coordinate without pretending the network is perfect.", "Distributed systems require careful thinking about latency, retries, idempotency, queues, timeouts, consistency, and failure. Professional engineers design for partial failure because one machine, one service, or one network path can fail while the rest of the system keeps running.", {"Define partial failure", "Explain idempotency", "Choose retry policy", "Identify consistency tradeoff", "Design observability signals"}, {"Trace a failed job retry", "Design a durable queue workflow", "Explain when eventual consistency is acceptable"}),
            lesson("l_adv_database_internals", "Database Internals and Indexes", "Learn how databases organize, search, and protect data.", "A database is not magic storage. It uses pages, indexes, query planning, transactions, logs, locks, and constraints. Understanding these pieces helps you design better schemas, troubleshoot slow queries, and avoid data corruption.", {"Explain B-tree index", "Read an EXPLAIN plan", "Describe transaction boundary", "Spot missing index", "Explain write/read tradeoff"}, {"Design an indexed table", "Compare full scan vs index lookup", "Write a transaction failure scenario"}),
            lesson("l_adv_compilers", "Compilers, Interpreters, and Language Design", "Understand how source code becomes executable behavior.", "Compilers and interpreters process source text through tokenizing, parsing, semantic analysis, optimization, and execution or code generation. Even if you never write a compiler professionally, this teaches precision, grammar, tooling, and debugging.", {"Tokenize source", "Describe parse tree", "Explain syntax vs semantic error", "Build tiny evaluator", "Report source location errors"}, {"Write tokens for an expression", "Design grammar for arithmetic", "Implement a tiny expression evaluator"}),
            lesson("l_adv_performance", "Performance Engineering", "Measure before optimizing and explain tradeoffs clearly.", "Performance work starts with measurement. You identify bottlenecks, profile hot paths, reduce unnecessary work, select better data structures, manage memory, and verify that the optimization actually improved the target metric without making code unsafe or unreadable.", {"Define target metric", "Profile hot path", "Compare algorithmic cost", "Measure before/after", "Document tradeoff"}, {"Optimize a loop", "Compare map vs vector lookup", "Write a performance report"}),
            lesson("l_adv_security_architecture", "Advanced Security Architecture", "Design secure systems instead of only patching bugs.", "Advanced secure engineering includes threat modeling, trust boundaries, secure defaults, audit logs, rate limiting, session design, secrets management, dependency review, and incident response. Security must be part of the architecture.", {"Identify trust boundary", "Design rate limiter", "Protect secrets", "Plan audit logs", "Write incident response step"}, {"Threat model an API", "Design login abuse controls", "Write secure deployment checklist"}),
            lesson("l_adv_ai_systems", "AI/RAG Systems Engineering", "Build AI features like software systems, not magic demos.", "AI/RAG systems involve ingestion, chunking, retrieval, reranking, context construction, answer generation, citation display, evaluation, privacy controls, and cost/performance monitoring. The professional skill is designing the whole system and proving it works.", {"Explain ingestion", "Explain retrieval", "Define eval set", "Add citation requirement", "Track failure modes"}, {"Design a RAG eval", "Trace bad retrieval", "Write a grounded-answer checklist"})
        },
        {
            exercise("ex_adv_job_queue", "Distributed Job Queue", "Advanced", "Implement or design a queue workflow with job states, retries, idempotency, and dead-letter handling. The runnable starter should print PASS_ADV_JOB_QUEUE when the workflow is correct.", "// states: queued, running, succeeded, failed, dead_letter\n", "Distributed systems, retries, idempotency"),
            exercise("ex_adv_btree_index", "B-Tree Index Trace", "Advanced", "Trace how a B-tree index finds a key and explain why it avoids scanning every row. The runnable starter should print PASS_ADV_BTREE_INDEX.", "// root page -> child page -> leaf page -> row pointer\n", "Database internals, indexing, query performance"),
            exercise("ex_adv_tokenizer", "Compiler Tokenizer", "Advanced", "Build a tokenizer for identifiers, numbers, operators, parentheses, and whitespace. The runnable starter should print PASS_ADV_TOKENIZER.", "// input: total = price * 2\n", "Compilers, parsing, language tooling"),
            exercise("ex_adv_rate_limiter", "Token Bucket Rate Limiter", "Advanced", "Model a token bucket limiter with capacity, refill rate, allow/deny decisions, and clear output. The runnable starter should print PASS_ADV_RATE_LIMITER.", "// capacity, tokens, refill, allow/deny\n", "Security architecture, API protection, reliability"),
            exercise("ex_adv_rag_pipeline", "RAG Retrieval Pipeline Trace", "Advanced", "Trace ingestion, chunking, retrieval, reranking, and answer context construction. The runnable starter should print PASS_ADV_RAG_PIPELINE.", "// document -> chunks -> search -> rerank -> cited context\n", "AI systems, retrieval, evaluation")
        },
        {
            quiz("q_adv_idempotency", "Why does idempotency matter in distributed job processing?", {"Retries can happen and must not corrupt state", "It makes UI colors brighter", "It removes all logs", "It only applies to CSS"}, 0, "Distributed systems often retry after timeouts or partial failures. Idempotent jobs can safely run more than once."),
            quiz("q_adv_btree", "What is the main purpose of a database index?", {"Speed up lookup/filter operations by avoiding unnecessary scans", "Replace all tables", "Make passwords visible", "Compile code"}, 0, "Indexes provide an efficient access path, often with write/storage tradeoffs."),
            quiz("q_adv_tokenizer", "What does a tokenizer usually output?", {"A stream of tokens with type and text", "A finished mobile app", "A database backup", "A network cable"}, 0, "Tokenizers convert raw source text into meaningful units for later parsing."),
            quiz("q_adv_profile", "What should happen before optimizing code?", {"Measure/profile to find the bottleneck", "Rewrite randomly", "Delete tests", "Ignore requirements"}, 0, "Optimization without measurement often wastes time or makes the wrong thing faster."),
            quiz("q_adv_rate_limit", "What does a rate limiter protect against?", {"Excessive or abusive request volume", "Syntax errors only", "Missing icons", "All compiler warnings"}, 0, "Rate limiting helps protect APIs and expensive services from abuse or overload."),
            quiz("q_adv_rag_eval", "Why do RAG systems need evaluation sets?", {"To prove retrieval and answers work on representative questions", "To avoid storing documents", "To remove citations", "To skip testing"}, 0, "RAG evals reveal bad retrieval, missing citations, hallucinations, and regressions.")
        },
        {
            project("p_adv_system_capstone", "Advanced Systems Capstone", "Senior Practice", "Build one advanced subsystem: job queue, tiny database, tokenizer/interpreter, secure API gateway, or RAG evaluation harness. Treat it as a course capstone with tests, trace output, and verification tokens.", {"Architecture note", "Runnable implementation", "Verification token", "Failure-mode notes", "Complexity/tradeoff explanation"}, {"Benchmark report", "Persistence layer", "Concurrent workers", "Security review", "Instructor rubric"})
        }
    });

    // v22 expands every module quiz bank so the Quiz Trainer has more depth per level.
    for (Module& module : modules)
    {
        ...
        module.quiz.push_back(quiz(module.id + "_q_checklist", "How should you treat a checklist item in this bootcamp?", {"As a vague reminder", "As a concrete task that needs evidence", "As optional decoration", "As a replacement for coding"}, 1, "v22 keeps checklist items evidence-based: define, implement or trace, verify, and record attempt evidence."));
        ...
        module.quiz.push_back(quiz(module.id + "_q_verify", "What does the Exercise Runner verifier look for in v22 starter exercises?", {"A matching expected output token", "A specific wallpaper", "A closed editor", "A portfolio screenshot"}, 0, "v22 guided course exercise packs print expected PASS_* tokens so the app can verify correctness after running code."));
        module.quiz.push_back(quiz(module.id + "_q_notes", "Why is progress evidence connected to the selected lesson/exercise context?", {"To keep evidence attached to the exact work being studied", "To hide mistakes", "To replace tests", "To remove quizzes"}, 0, "Contextual progress evidence makes review useful because it shows what ran, what verified, and what still needs work."));
